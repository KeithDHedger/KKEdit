							...
							if(newstr!=NULL)
								{
									...
									g_signal_connect(G_OBJECT(menuitem),"activate",G_CALLBACK(gotoLineSavePos),(gpointer)(long)fd->line);
								}
						}
					else
						{
...
VISIBLE void openHelp(GtkWidget *widget,gpointer data)
{
	...
	if((long)data==0)
		...
		sinkReturn=asprintf(&thePage,"http://keithdhedger.github.io/KKEdit/help.%s.html",lang);
	...
	if((long)data==0)
		...
		sinkReturn=asprintf(&thePage,"%s http://keithdhedger.github.io/KKEdit/help.%s.html",browserCommand,lang);
...
void setPrefs(GtkWidget *widget,gpointer data)
{
	...
	bool		*bools[MAXPREFSWIDGETS]={&indent,&lineNumbers,&lineWrap,&highLight,&noSyntax,&singleUse,&onExitSaveSession,&noDuplicates,&noWarnings,&readLinkFirst,&autoShowComps,&nagScreen,&useGlobalPlugMenu,&autoSearchDocs,&showWhiteSpace,&inWindow,&showMenuIcons};
